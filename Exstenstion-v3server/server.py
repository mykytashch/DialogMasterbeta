...
def get_message():
    ...
    if 0 <= index < len(messages):
        return jsonify({'messages': messages[index:]})  # Возвращаем только непрочитанные сообщения
...
def store_response():
    ...
    sender = response.get('sender')  # Получаем отправителя из запроса

    # Сохраняем ответ пользователя
    state['responses'].append({'message': message, 'sender': sender})

    # Увеличиваем индекс сообщения
    state['currentMessageIndex'] += 1
    logging.debug(f"Stored response: {message} (sender: {sender})")
